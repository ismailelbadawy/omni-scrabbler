...
NodeMC* MonteCarlo::getHighestUCB(NodeMC* root)
{
	...
	{
		{
			...
			for (int i = k; i < x; i++)
			{
				AllNodes.insert(AllNodes.end(), AllNodes[i]->children.begin(), AllNodes[i]->children.end());
				k = i;
			}
		}
	}
	...
	{
		if (AllNodes[i]->nodeState.UCB>max)
		{
			maxNode = AllNodes[i];
			max = AllNodes[i]->nodeState.UCB;
		}
	}
	return maxNode;
}
